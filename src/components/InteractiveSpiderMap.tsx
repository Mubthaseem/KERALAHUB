...
import { ShieldAlert, AlertTriangle, Tent, Phone, Users, MapPin, Eye, ThumbsUp, Filter, Crosshair } from 'lucide-react';

interface InteractiveSpiderMapProps {
...
// Custom Leaflet Icons styled as Spider Radar Beacons
const createSpiderIcon = (type: 'sos' | 'flood' | 'camp') => {
  let iconHtml = '';
  if (type === 'sos') {
    iconHtml = `
      <div className="spider-pin-sos">
    ...
    iconHtml = `
      <div className="spider-pin-flood">
  ...
  return L.divIcon({
    html: iconHtml,
    className: 'custom-leaflet-spider-pin',
    iconSize: [32, 32],
    iconAnchor: [16, 16]
  });
};
...
// Component to dynamically re-center map when district is selected
...
// Map click handler to trigger new report/SOS
...
export const InteractiveSpiderMap: React.FC<InteractiveSpiderMapProps> = ({
  sosRequests,
  disasterReports,
  reliefCamps,
  selectedDistrict,
  ...
  return (
    <div className="relative w-full h-[calc(100vh-140px)] min-h-[550px] bg-slate-100 border border-slate-200 rounded-xl overflow-hidden shadow-lg flex flex-col md:flex-row">
      ...
      <div className="absolute top-4 left-4 z-[500] flex flex-wrap items-center gap-2 bg-white/95 backdrop-blur-md p-2 rounded-xl border border-slate-200 shadow-md">
        <div className="text-xs font-mono font-bold text-slate-700 px-2 flex items-center gap-1 border-r border-slate-200 pr-3">
        ...
        >
          <span>🆘</span>
          <span>SOS Beacons ({filteredSOS.length})</span>
        </button>
        ...
        >
          <span>🌊</span>
          <span>Flood Hazards ({filteredReports.length})</span>
        </button>
        ...
        >
          <span>⛺</span>
          <span>Relief Camps ({filteredCamps.length})</span>
        </button>
      </div>
      ...
      <div className="absolute bottom-4 left-4 right-4 md:right-auto z-[500] flex items-center gap-1.5 overflow-x-auto no-scrollbar bg-slate-900/90 backdrop-blur-md p-2 rounded-xl border border-slate-800 text-xs text-white max-w-full md:max-w-2xl">
        <span className="font-mono font-bold text-red-400 text-[11px] whitespace-nowrap pl-1">
          HOTSPOTS:
        </span>
      ...
      <div className="w-full h-full relative">
        ...
        >
          ...
          {/* Render SOS Pin Markers */}
          {showSOS &&
            filteredSOS.map((sos) => (
              <Marker
                key={sos.id}
                position={[sos.lat, sos.lng]}
                icon={createSpiderIcon('sos')}
              >
                <Popup className="spider-map-popup">
                  <div className="p-1 max-w-xs font-sans">
                    <div className="flex items-center justify-between gap-2 border-b border-slate-200 pb-1.5 mb-2">
                      <span className="bg-red-100 text-red-700 font-bold text-[10px] px-2 py-0.5 rounded font-mono uppercase">
                    ...
                    <div className="bg-slate-50 p-2 rounded-lg border border-slate-200 text-xs mb-3 space-y-1 font-mono">
                      ...
                      {sos.people_count && (
                        <div className="flex items-center gap-1 text-slate-700">
                          ...
                          <span>Stranded People: <strong>{sos.people_count}</strong></span>
                        </div>
                      )}
                    </div>
                    ...
                    >
                      ...
                      <span>CALL REFUGEE ({sos.phone})</span>
                    </a>
                  </div>
                </Popup>
              </Marker>
          ...
          {showFloods &&
            filteredReports.map((report) => (
              <Marker
                ...
                icon={createSpiderIcon('flood')}
              >
                <Popup className="spider-map-popup">
                  <div className="p-1 max-w-xs font-sans">
                    <div className="flex items-center justify-between gap-2 border-b border-slate-200 pb-1.5 mb-2">
                      <span className="bg-blue-100 text-blue-700 font-bold text-[10px] px-2 py-0.5 rounded font-mono uppercase">
                    ...
                    {report.image_url && (
                      <div className="relative mb-2 rounded-lg overflow-hidden border border-slate-200 max-h-36 group">
                        ...
                        <button
                          ...
                          className="absolute inset-0 bg-slate-900/40 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition text-xs font-mono gap-1"
                        >
                    ...
                    <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-100">
                      <span className="text-slate-500 font-mono">{report.location_name}</span>
                      <button
          ...
          {showCamps &&
            filteredCamps.map((camp) => (
              <Marker
                ...
                icon={createSpiderIcon('camp')}
              >
                <Popup className="spider-map-popup">
                  <div className="p-1 max-w-xs font-sans">
                    <div className="flex items-center justify-between gap-2 border-b border-slate-200 pb-1.5 mb-2">
                      <span className="bg-emerald-100 text-emerald-800 font-bold text-[10px] px-2 py-0.5 rounded font-mono uppercase">
                    ...
                    <div className="mb-3 text-[11px]">
                      <span className="font-bold text-slate-700 block mb-1 font-mono">Urgent Supplies Needed:</span>
                    ...
                    >
                      <Phone className="w-3.5 h-3.5" />
                      <span>CALL CAMP INCHARGE ({camp.phone})</span>
                    </a>
                  </div>
                </Popup>
              </Marker>
          ...
          <div className="max-w-3xl w-full bg-white rounded-2xl overflow-hidden shadow-2xl border border-slate-200 p-2">
            <div className="flex justify-end p-2">
              <button
                ...
                className="text-slate-500 hover:text-slate-900 font-mono font-bold text-sm bg-slate-100 px-3 py-1 rounded-lg"
              >
                Close ✕
              </button>
            </div>
