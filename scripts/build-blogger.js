...
// 2. Wrap script contents in CDATA for Blogger SAX parser
html = html.replace(/<script([^>]*)>([\s\S]*?)<\/script>/gi, (match, attrs, code) => {
  if (!code.trim()) return match;
  // Replace any CDATA end tags inside code if any
  const safeCode = code.replace(/\]\]>/g, ']]]]><![CDATA[>');
  return `<script${attrs}>\n/*<![CDATA[*/\n${safeCode}\n/*]]>*/\n</script>`;
});

// 3. Wrap style contents in CDATA
...
// 4. Construct valid Blogger XML template structure
...
<head>
  ...
  ${html.substring(html.indexOf('<head>') + 6, html.indexOf('</head>'))}
</head>
<body>
  ${html.substring(html.indexOf('<body'), html.indexOf('</body>') + 7)}
...
console.log('Successfully generated 100% valid Blogger XML: blogger_theme.xml');
